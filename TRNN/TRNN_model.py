...
class ChildSumTreeLSTM(nn.Module):
    def __init__(self, cuda, in_dim, mem_dim, num_classes):
        ...
        self.output_module = nn.Linear(self.mem_dim, self.num_classes)

    def call(self, inputs, child_c, child_h):
        # 注意call被改动了
        """
        ...
        fx = F.torch.unsqueeze(self.fx(inputs), 0)

        f = torch.stack([self.fh(child_hi)+fx for child_hi in child_h], 0)
        f = torch.sigmoid(f)
        ...
        # print('c', c.shape)
        # print('h', c.shape)
        return F.torch.unsqueeze(c, 0), F.torch.unsqueeze(h, 0)

    def tree_rnn(self, tree, training = False):
        """
        ...
        # add singleton dimension for future call to call
        # embs = F.torch.unsqueeze(self.emb(inputs),1)

        if tree.num_children == 0:    # 如果是叶节点，初始化child_c, child_h = 0
            ...
            for idx in range(tree.num_children):
                if tree.children[idx].state == None: # 如果有一个儿子的state是None， 递归到那个孩子
                    self.tree_rnn(tree.children[idx], training)
                    continue

                child_c[idx] = tree.children[idx].state[0]
                child_h[idx] = tree.children[idx].state[1]

            # 如果所有孩子都有state, 更新state
        tree.state = self.call(tree.x, child_c, child_h)

        return tree.state

    def forward(self, tree, training = False):
        state = self.tree_rnn(tree, training)
        output = self.output_module(state[1])
        return output
